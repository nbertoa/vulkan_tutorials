...
// They explicitly define the access types and buffer and image subresource ranges that are included 
// in the access scopes of a memory dependency that is created by a synchronization command that includes them.
//
...
// At any given time, the data representing an image subresource in memory exists in a particular layout 
// which is determined by the most recent layout transition that was performed on that image subresource.
...
public:
    ...
    //     COLOR_ATTACHMENT_WRITE_BIT, DEPTH_STENCIL_ATTACHMENT_READ_BIT, DEPTH_STENCIL_ATTACHMENT_WRITE_BIT,
    //     TRANSFER_READ_BIT, TRANSFER_WRITE_BIT, HOST_WRITE_BIT, MEMORY_READ_BIT, MEMORY_WRITE_BIT,
    //     CONDITIONAL_RENDERING_READ_BIT_EXT, TRANSFORM_FEEDBACK_WRITE_BIT_EXT, TRANSFORM_FEEDBACK_COUNTER_READ_BIT_EXT,
    //     TRANSFORM_FEEDBACK_COUNTER_WRITE_BIT_EXT, COMMAND_PROCESS_READ_BIT_NVX, COLOR_ATTACHMENT_READ_NONCOHERENT_BIT_EXT,
    //     SHADING_RATE_IMAGE_READ_BIT_NV, ACCELERATION_STRUCTURE_READ_BIT_NV, ACCELERATION_STRUCTURE_WRITE_BIT_NV,
    ...
    // Some synchronization commands take sets of access types as parameters to define the access scopes of a memory dependency.
    // If a synchronization command includes a source access mask, its first access scope only includes accesses via 
    // the access types specified in that mask.
    // Similarly, if a synchronization command includes a destination access mask, its second access scope only includes 
